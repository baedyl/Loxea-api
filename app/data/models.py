...
class UserChassisLink(SQLModel, table=True):
    ...
    chassis_id: Optional[int] = Field(
        default=None, foreign_key="chassis.id", primary_key=True
    )
...
class Chassis(Base, table=True):
    ...
    users: List["User"] = Relationship(
        back_populates="chassis", link_model=UserChassisLink
    )
...
class User(Base, table=True):
    ...
    chassis: List[Chassis] = Relationship(
        back_populates="users", link_model=UserChassisLink
    )
    assistances: List["Assistance"] = Relationship(back_populates="user")
    accident_declarations: List["AccidentDeclaration"] = Relationship(
        back_populates="user"
    )
...
class Feedback(Base, table=True):
    ...
    category_id: Optional[int] = Field(
        default=None, foreign_key="feedback_categories.id"
    )
    message: str
